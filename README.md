...
- Dangerous git (`push --force`, `reset --hard` to `main`/`master`, `clean -fdx`)
...
`fixtures/sample-session.jsonl` mixes safe reads/tests with intentional hits: `rm -rf`, force-push, hard reset to main, `git clean -fdx`, `DROP`/`TRUNCATE`, `mkfs`/`dd`, `curl|bash`, `chmod 777`, and secret-shaped outputs.
...
| git | `git push --force` / `-f` / `--force-with-lease`, `git reset --hard` … `main`\|`master`, `git clean -fdx` | critical / high |
